...
class TabcotizacionesDetalleController extends Controller
{
    protected $_tabCotizacionesDetalle;

    public function __construct(TabCotizacionesDetalleRepositoryInterface $tabCotizacionesDetalle)
    {
        $this->_tabCotizacionesDetalle = $tabCotizacionesDetalle;
    }

    public function getAll()
    {
        try {
            $getAll = $this->_tabCotizacionesDetalle->getAll();
    ...
    {
        try {
            $getById = $this->_tabCotizacionesDetalle->getByID($id);
    ...
    {
        try {
            $getById = $this->_tabCotizacionesDetalle->getByIdDetalle($id);
            return ApiResponseHelper::sendResponse($getById, 'Cotización obtenida', 200);
        } catch (Exception $ex) {
            return ApiResponseHelper::sendResponse($ex, 'No se pudo obtener el registro', 500);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($elementosAAgregar as $elemento) {
                $this->_tabCotizacionesDetalle->store($elemento); // Ahora sí estás pasando un array
            }
            $idSolicitudD = $elementosAAgregar->first()['id_solicitud_detalle'];
            $solicitudD = TabSolicitudDetalle::find($idSolicitudD);
            ...
            DB::commit();
            return ApiResponseHelper::sendResponse(null, 'Cotización creada correctamente', 201);
        } catch (Exception $ex) {
            DB::rollBack();
    ...
    public function update(Request $cat)
    {
        ...
        try {
            ...
            foreach ($elementosAActualizar as $elemento) {
                $this->_tabCotizacionesDetalle->update($elemento,$elemento["id"]); // Ahora sí estás pasando un array
            }
            DB::commit();
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            foreach ($elementosAEliminar as $elemento) {
                $this->_tabCotizacionesDetalle->delete($elemento['id']); // Ahora sí estás pasando un array
            }
