    ...
    public function cargarArchivoCompleto(Request $request, $detalleArchivo)
    {
        ...
        foreach ($filasConDatosSinEncabezado as $record) {
            ...
            DB::table('tab_archivo_completos')->insert([
                ...
                'grupo_articulos' => str_replace(['-',''],'0',str_replace(['$', ' '], '', $record[4])),
                'libre_utilizacion' => str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[5])),
                'en_control_calidad' =>  str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[6])),
                'bloqueado' =>  str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[7])),
                'valor_libre_util' => str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[8])),
                'valor_insp_cal' =>  str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[9])),
                'valor_stock_bloq' =>  str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[10])),
                'cantidad_total' =>  str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[11])),
                'importe_unitario' => str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[12])),
                'importe_total' =>  str_replace(['-',''],'0',str_replace(['$', ' ',','], '', $record[13])),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
